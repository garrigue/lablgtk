...
let main () =
  ...
  let window =
    new GWindow.window title: "GTK Entry" width: 200 height: 100 in
  window#connect#destroy callback:Main.quit;

  let vbox = new box `VERTICAL packing: window#add in

  let entry = new entry max_length: 50 packing: vbox#add in
  ...
  let hbox = new box `HORIZONTAL packing: vbox#add in

  let check =
    new check_button label: "Editable" active: true packing: hbox#pack in
  check#connect#toggled callback:(fun () -> entry_toggle_editable check entry);

  let check =
    new check_button label: "Visible" active: true packing: hbox#pack in
