...
let parse_file fname =
  let ch =
    try open_in fname with Sys_error _ -> raise (!file_not_found fname) in
  try
...
let prove_attrib dtd attr aname (atype,adef) accu =
  ...
  | DTDRequired, None ->
      raise (Prove_error (RequiredAttribute aname))
...
let rec do_prove dtd = function
  ...
  | Element (tag,attr,childs) ->
      let utag = String.uppercase tag in
      let uattr =
	List.map (fun (aname,aval) -> String.uppercase aname , aval) attr in
      prove_child dtd (Some utag);
      Stack.push (dtd.curtag,dtd.current) dtd.state;
      let elt =
	try Hashtbl.find dtd.elements utag
	with Not_found -> raise (Prove_error (UnexpectedTag tag)) in
      let ahash =
	try Hashtbl.find dtd.attribs utag with Not_found -> empty_hash in
      dtd.curtag <- tag;
      dtd.current <- elt;
...
let parse_error (msg,pos) =
  ...
  if pos.emin = pos.emax then
    sprintf "%s line %d character %d"
      (parse_error_msg msg) pos.eline (pos.emin - pos.eline_start)
  else
    sprintf "%s line %d characters %d-%d"
      (parse_error_msg msg) pos.eline
...
let to_string = function
  | DTDAttribute (tag,aname,atype,adef) ->
	...
	| DTDFixed s -> sprintf "#FIXED \"%s\"" s
      in
      sprintf "<!ATTLIST %s %s %s %s>"
	tag aname (atype_to_string atype) (adefault_to_string adef)
  | DTDElement (tag,etype) ->
