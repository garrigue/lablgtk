...
type query =
  { id : int; signal_name : string; itype : string; flags : int;
    return : string; params : string array
  }
...
(*
external signal_new : string -> g_type -> Gobject.signal_type list -> unit =
  "ml_g_signal_new_me"
...
let connect ~sgn: ((sgn : ('a, _) t)) ~callback ?after (obj : 'a obj) =
  connect_aux ~name: sgn.name ~marshaller: sgn.marshaller ~callback ?after
    obj

let connect_property ~prop: ((prop : ('a, _) property)) ~callback
                     (obj : 'a obj) =
  let name = "notify::" ^ prop.Gobject.name in
  let marshaller = marshal1 prop.conv name
  in connect_aux ~name ~marshaller ~callback obj
