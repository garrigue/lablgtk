...
let connect : sgn:('a, _) t -> callback: _ -> ?after: _ -> 'a obj -> _ =
  fun ~sgn ~callback ?(after=false) obj ->
    connect obj ~name:sgn.name ~callback:(sgn.marshaller callback) ~after
external disconnect : 'a obj -> id -> unit
  = "ml_gtk_signal_disconnect"
external emit : 'a obj -> name:string -> unit = "ml_gtk_signal_emit_by_name"
let emit (obj : 'a obj) ~(sgn : ('a,unit->unit) t) =
