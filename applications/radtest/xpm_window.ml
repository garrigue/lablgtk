...
let target_table = [|   { target = "STRING"; flags = []; info = 0}  |]


let xpm_window () =
  let source_drag_data_get classe _ (data : selection_data) _ _ =
    data#set type:data#target format:0 classe length:(String.length classe) in
  ...
  ev#misc#drag#source_set mod:[`BUTTON1] target_table 1 [`COPY];
  ev#misc#drag#source_set_icon colormap:window#misc#style#colormap 
    gdk_pix; 
