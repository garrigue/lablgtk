...
let next_token = Propcc_lexer.next_token
let token_pp = Propcc_lexer.token_pp
...
let ident tokens = match peek tokens with
  ...
  | _ -> raise @@ Parse_error (peek tokens)
let verbatim tokens = match peek tokens with
  | Verbatim text -> drop tokens; text
  | _ -> raise @@ Parse_error (peek tokens)
let kwd tok tokens =
...
(* specific props parser *)
let may_colon parser ~message def tokens =
  try
    kwd Colon tokens;
    require ~message parser tokens
  with Parse_error _ -> def

let may_name name tokens =
  let message = "( ident ) expected" in
  try
    kwd Lparen tokens;
    let id = require ~message ident tokens in
    require kwd ~message Rparen tokens;
    id
  with Parse_error _ -> (camlize name)

let simple_attr tokens =
  let message = "/ ident" in
  kwd Slash tokens;
  require ~message ident tokens

(* Some examples of multi word attributes: [Construct Only] [No Wrap] *)
let next_attr tokens =
  let id = simple_attr tokens in
  let ids = star ~acc: [id] ident tokens in
  String.concat ~sep:"" ids

let label_type2 id tokens =
  let message = ": ident expected" in
  try
    kwd Colon tokens;
    id, require ~message ident tokens
  with Parse_error _ -> "", id

let label_type tokens =
  let id = ident tokens in
  label_type2 id tokens

type marshal =
    Function of string | Types of (string list * string list * string)

let return_type (l, types) tokens =
  let message = "-> ident expected" in
  try
    kwd Left_arrow tokens;
    let ret = require ~message ident tokens in
    Types (l, types, ret)
  with Parse_error _ -> Types (l, types, "")

let marshaller tokens = match peek tokens with
  | String func -> drop tokens; Function func
  | Colon -> drop tokens;
    let types = star label_type tokens in
    return_type (List.split types) tokens
  | _ -> Types ([], [], "")
...
let field tokens = match peek tokens with
  (* property *)
  | String name -> drop tokens;
    let message = {| "string" (ident)? ident : ident (/ ident )* expected |} in
    let mlname = may_name name tokens in
    let gtype = require ~message ident tokens in
    require ~message kwd Colon tokens;
    let attr0 = require ~message ident tokens in
    let attrs = star ~acc: [attr0] next_attr tokens in
  ...
  (* method *)
  | Method -> drop tokens;
    let message = {|method ident (: "string")? (ident)* expected|} in
    let name = require ~message ident tokens in
    let ty = may_colon string ~message "unit" tokens in
    let attrs = star simple_attr tokens in
  ...
  (* signal *)
  | Signal -> drop tokens;
    let message = "signal .. " in
    let name = require ~message ident tokens in
    let m = require ~message marshaller tokens in
    let l = star simple_attr tokens in
...
let read_pair tokens =
  let cls = ident tokens in
  cls, may_parser string (camlize cls) tokens

let qualifier tokens =
  let id = ident tokens in
  id, may_parser string "" tokens
...
let process_phrase tokens = match peek tokens with

  | Ident "class" -> drop tokens;
    let message = "class name gtk_name? attr* (: parent)? {..}" in
    let name = require ~message ident tokens in
    let gtk_name = may_parser string (!prefix ^ name) tokens in
    let attrs = star qualifier tokens in
    let parent = may_colon ident ~message "" tokens in
    let text = require ~message verbatim tokens in

    let text_tokens = Tokens.make @@ Lexing.from_string text in
    let fields = star field text_tokens in

    if List.exists attrs ~f:
        (fun (x,_) -> not (List.mem x ~set:class_qualifiers))
    then failwith "bad qualifier";
  ...
  | Ident "initializer" -> drop tokens;
    let func = require ~message: "initializer (string) expected" string tokens in
    initializers := !initializers @ [func]

  | Ident "header" -> drop tokens;
    let header = require ~message: "header (string) expected" verbatim tokens in
    headers := !headers @ [header]

  | Ident "oheader" -> drop tokens;
    let oheader = require ~message: "oheader (string) expected" verbatim tokens in
    oheaders := !oheaders @ [oheader]

  | Ident "prefix" -> drop tokens;
    let id = require ~message: "prefix (string) expected" string tokens in
    prefix := id

  | Ident "tagprefix" -> drop tokens;
    let id = require ~message: "tagprefix (string) expected" string tokens in
    tagprefix := id

  | Ident "conversions" -> drop tokens;
    let message = "conversions pre1? pre2? {..}" in
    let pre1 = may_parser string "" tokens in
    let pre2 = may_parser string pre1 tokens in
    let text = require ~message verbatim tokens in
    (* Create inner lexer *)
    let text_tokens = Tokens.make @@ Lexing.from_string text in
    let l = star read_pair text_tokens in

    List.iter l ~f:(fun (k,d) ->
        Hashtbl.add conversions ~key:(pre1^k) ~data:(if pre2="" then d else pre2^"."^d))

  | Ident "classes" -> drop tokens;
    let text = require ~message: "classes {..}" verbatim tokens in
    (* create inner lexer *)
    let text_tokens = Tokens.make @@ Lexing.from_string text in
    let l = star read_pair text_tokens in
  ...
  | Ident "boxed" -> drop tokens;
    let text = require verbatim tokens in
    (* create inner lexer *)
    let text_tokens = Tokens.make @@ Lexing.from_string text in
    let l = star read_pair text_tokens in
...
let process_file f =
  ...
  begin try
    while true do process_phrase tokens done
  with | End_of_file -> ()
       | exn -> Printf.eprintf "Exception %s in file `%s' before char %d\n"
                  (Printexc.to_string exn) f (Tokens.position tokens)
  end;
  (* Preproccess *)
...
let () = if !Sys.interactive then () else Printexc.print main ()
