...
let rec search_pos_type t :pos :env =
  let search_opt_list tl =
      List.iter tl fun:
      begin function _,None -> ()
      | _,Some t -> search_pos_type t :pos :env
      end in
  if in_loc :pos t.ptyp_loc then
  begin (match t.ptyp_desc with
    Ptyp_any
  | Ptyp_var _ -> ()
  | Ptyp_variant var -> search_opt_list var.pvar_max
  | Ptyp_arrow (_, t1, t2) ->
  ...
  | Ptyp_poly (t, _) -> search_pos_type :pos :env t);
  raise Not_found
  end
...
let shown_modules = new Ohashtbl.c size:17
...
let edit_source :file :path :sign =
  match sign with
	...
	try
	  ...
	  if Filename.check_suffix file suffix:".ml" then
...
let top_widgets : GWindow.window list ref = ref []

let rec view_signature sign ?:title ?:path ?:env [< !start_env >] =
  let env =
...
and view_expr_type t ?:title ?:path ?:env ?:name [< "noname" >] =
  let title =
    match title, path with Some title, _ -> title
    | None, Some path -> string_of_path path
...
let rec search_pos_structure str :pos =
  List.iter str fun:
  begin function
...
and search_pos_class_expr cl :pos =
  if in_loc cl.cl_loc :pos then begin
    ...
    | Tclass_fun (pat, iel, cl) ->
	search_pos_pat pat :pos env:pat.pat_env;
	List.iter iel fun:(fun (_,exp) -> search_pos_expr exp :pos);
	search_pos_class_expr cl :pos
    | Tclass_apply (cl, el) ->
	search_pos_class_expr cl :pos;
	List.iter el fun:(search_pos_expr :pos)
...
and search_pos_expr exp :pos =
  ...
  | Texp_when (a, b) ->
      search_pos_expr a :pos; search_pos_expr b :pos
  | Texp_constraint exp -> search_pos_expr exp :pos
...
and search_pos_pat pat :pos :env =
...
and search_pos_module_expr m :pos =
